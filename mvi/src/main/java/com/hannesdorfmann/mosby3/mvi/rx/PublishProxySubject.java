 ...
 */
public final class PublishProxySubject<T> extends Subject<T> {
   ...
   */
  public static <T> PublishProxySubject<T> create() {
    return new PublishProxySubject<T>();
  }
   ...
   * @since 2.0
   */
  @SuppressWarnings("unchecked") PublishProxySubject() {
    subscribers = new AtomicReference<PublishDisposable<T>[]>(EMPTY);
  }

  @Override public void subscribeActual(Observer<? super T> t) {
  ...
  @SuppressWarnings("unchecked") @Override public void onError(Throwable t) {
  ...
  @SuppressWarnings("unchecked") @Override public void onComplete() {
    for (PublishDisposable<T> s : subscribers.get()) {
      s.onComplete();
    }
  }
  ...
  static final class PublishDisposable<T> extends AtomicBoolean implements Disposable {
    ...
    /** The subject state. */
    final PublishProxySubject<T> parent;
     ...
     */
    PublishDisposable(Observer<? super T> actual, PublishProxySubject<T> parent) {
      this.actual = actual;
      this.parent = parent;
    }
