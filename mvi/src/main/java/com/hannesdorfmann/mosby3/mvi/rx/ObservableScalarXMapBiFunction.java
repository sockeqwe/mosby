 ...
 */
final class ObservableScalarXMapBiFunction {

    /** Utility class. */
    private ObservableScalarXMapBiFunction() {
    ...
    public static <T1, T2,  R> boolean tryScalarXMapSubscribe(ObservableSource<T1> source,
            Observer<? super R> observer,
            BiFunction<? super T1, ? super T2, ? extends ObservableSource<? extends R>> mapper, T2 value) {
        if (source instanceof Callable) {
            T1 t;

            try {
                t = ((Callable<T1>)source).call();
            } catch (Throwable ex) {
            ...
            try {
                r = ObjectHelper.requireNonNull(mapper.apply(t, value), "The mapper returned a null ObservableSource");
            } catch (Throwable ex) {
                Exceptions.throwIfFatal(ex);
                EmptyDisposable.error(ex, observer);
                return true;
