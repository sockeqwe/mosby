 ...
 * {@link Observer}.
 * <p>
 * <img width="640" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/S.BehaviorSubject.png"
 * alt="">
 * <p>
 * Example usage:
 * <p>
 ...
 */
public final class BehaviorProxySubject<T> extends Subject<T> {
  ...
  /**
   * Creates a {@link BehaviorProxySubject} without a default item.
   *
   * @param <T> the type of item the Subject will emit
   * @return the constructed {@link BehaviorProxySubject}
   */
  public static <T> BehaviorProxySubject<T> create() {
    return new BehaviorProxySubject<T>();
  }

  /**
   * Creates a {@link BehaviorProxySubject} that emits the last item it observed and all subsequent
   * items to each
   ...
   * {@link BehaviorProxySubject} has not yet observed any items from its source {@code Observable}
   * @return the constructed {@link BehaviorProxySubject}
   */
  public static <T> BehaviorProxySubject<T> createDefault(T defaultValue) {
    return new BehaviorProxySubject<T>(defaultValue);
  }
   ...
   * @since 2.0
   */
  @SuppressWarnings("unchecked") BehaviorProxySubject() {
   ...
   * @since 2.0
   */
  BehaviorProxySubject(T defaultValue) {
    this();
   ...
   */
  @SuppressWarnings("unchecked") public T[] getValues(T[] array) {
    Object o = value.get();
  ...
  static final class BehaviorDisposable<T> implements Disposable, NonThrowingPredicate<Object> {
    ...
    final BehaviorProxySubject<T> state;
    ...
    BehaviorDisposable(Observer<? super T> actual, BehaviorProxySubject<T> state) {
      this.actual = actual;
      this.state = state;
    }
    ...
    void emitFirst() {
      ...
      synchronized (this) {
        ...
        BehaviorProxySubject<T> s = state;
